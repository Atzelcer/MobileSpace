...
AAventuraManager::AAventuraManager()
{
    ...
    bJefeDerrotadoNotificado = false;
    ...
    SonidoCarga = LoadObject<USoundWave>(nullptr, TEXT("SoundWave'/Game/BOSS_SOUNDS/PantallaCarga.PantallaCarga'"));
    IntervaloControlAtaque = 0.25f;
    bJugadorMoviendose = false;
    VelocidadTeletransporte = 550.f;
}
...
void AAventuraManager::SetupFixedCamera()
{
    ...
    if (PC) PC->SetViewTarget(FixedCamera);
}
...
{
    APlayerController* PC = UGameplayStatics::GetPlayerController(GetWorld(), 0);
    if (!PC || !AudioComp_SonidoCarga || !SonidoCarga) return;
    ...
    if (bActivarSonido)
    {
        ...
        GetWorld()->GetTimerManager().SetTimer(
            ...
            [this]() { ActivarEfectoSonidoPantallaCarga(false); },
            4.5f, false);
    }
    else
    {
...
void AAventuraManager::GenerarOleada()
{
    ...
    else
    {
        ...
        for (int32 i = 0; i < CantidadPorOleada; i++)
        {
            ...
            ShipFactory->CrearNave(GetWorld(), TipoSeleccionado, SpawnLocation, FRotator::ZeroRotator);
        }
    }
...
void AAventuraManager::GenerarEnjambre()
{
    ...
    switch (NivelActual)
    {
    case 1: SwarmRows = 3; SwarmColumns = 4; break;
    case 2: SwarmRows = 4; SwarmColumns = 3; break;
    case 3: SwarmRows = 4; SwarmColumns = 4; break;
    case 4: SwarmRows = 4; SwarmColumns = 5; break;
    case 5: SwarmRows = 5; SwarmColumns = 4; break;
    case 6: SwarmRows = 5; SwarmColumns = 5; break;
    default: SwarmRows = 4; SwarmColumns = 4; break;
    }
    ...
    for (int32 Row = 0; Row < SwarmRows; Row++)
    {
        for (int32 Column = 0; Column < SwarmColumns; Column++)
        {
            ...
            AShip_X* NewShip = ShipFactory->CrearNave(
                GetWorld(),
                TipoSeleccionado,
                EntryPosition,
    ...
    if (Enemigos.Num() == 0 && !bOleadasCompletadas)
    {
        ...
        else
        {
            ...
            GetWorldTimerManager().SetTimer(
                BossTimerHandle,
                this,
                &AAventuraManager::SpawnJefeDelNivel,
                3.5f,
                false);
        }
    }
    else if (bOleadasCompletadas && NivelActual != 7)
    {
        TArray<AActor*> BossCheck;
        UGameplayStatics::GetAllActorsOfClass(GetWorld(), ABoss::StaticClass(), BossCheck);

        if (bJefeActivo && BossCheck.Num() == 0 && !bJefeEliminado)
        {
            bJefeEliminado = true;
            bJefeActivo = false;
            ...
            GetWorldTimerManager().SetTimer(
                WaveTimerHandle,
                this,
                &AAventuraManager::SpawnPortalFinal,
                1.0f,
                false);
...
void AAventuraManager::RegistrarEventosBoss(ABoss* Boss)
{
    if (!Boss) return;
    Boss->OnDestroyed.AddDynamic(this, &AAventuraManager::OnBossDestroyed);
}

void AAventuraManager::OnBossDestroyed(AActor* DestroyedActor)
{
    NotificarBossMuerto();
}

void AAventuraManager::NotificarBossMuerto()
{
    if (bJefeDerrotadoNotificado)
        return;

    bJefeDerrotadoNotificado = true;
    bJefeActivo = false;
    bJefeEliminado = true;
...
void AAventuraManager::SpawnPortalFinal()
{
    ...
    FVector SpawnLocation(850.f, 0.f, -430.f);
    FRotator SpawnRotation = FRotator::ZeroRotator;

    FActorSpawnParameters Params;
    Params.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;

    GetWorld()->SpawnActor<AMegaPortal>(
...
void AAventuraManager::GenerarOleadaObstaculos()
{
    ...
    for (int32 i = 0; i < CantidadPorOleadaObstaculos; i++)
    {
        ...
        AMegaObstaculo* Nuevo = GetWorld()->SpawnActor<AMegaObstaculo>(
            AMegaObstaculo::StaticClass(),
            SpawnLocation,
        ...
        if (Nuevo)
            Nuevo->SetVelocidadMovimiento(FMath::RandRange(300.f, 900.f));
    }
    ...
    GetWorld()->GetTimerManager().SetTimerForNextTick(
        this,
        &AAventuraManager::ComprobarOleadaObstaculos);
}
...
void AAventuraManager::ComprobarOleadaObstaculos()
{
    ...
    {
        GetWorld()->GetTimerManager().SetTimer(
            TimerHandle_ComprobarObstaculos,
            this,
            &AAventuraManager::ComprobarOleadaObstaculos,
            1.0f,
            false);
...
void AAventuraManager::TeletransportarJugador()
{
    ...
    PawnJugador->SetActorLocation(FVector(-2800.f, 0.f, 300.f));
...
void AAventuraManager::SpawnJefeDelNivel()
{
    ...
    FVector SpawnLocation(1400.0f, 0.0f, 300.0f);
    ...
    switch (TipoJefe)
    {
    case 1: NuevoJefe = GetWorld()->SpawnActor<ABoss_1>(SpawnLocation, SpawnRotation, SpawnParams); break;
    case 2: NuevoJefe = GetWorld()->SpawnActor<ABoss_2>(SpawnLocation, SpawnRotation, SpawnParams); break;
    case 3: NuevoJefe = GetWorld()->SpawnActor<ABoss_3>(SpawnLocation, SpawnRotation, SpawnParams); break;
    case 4: NuevoJefe = GetWorld()->SpawnActor<ABoss_4>(SpawnLocation, SpawnRotation, SpawnParams); break;
    case 5: NuevoJefe = GetWorld()->SpawnActor<ABoss_5>(SpawnLocation, SpawnRotation, SpawnParams); break;
    case 6: NuevoJefe = GetWorld()->SpawnActor<ABoss_6>(SpawnLocation, SpawnRotation, SpawnParams); break;
    default: NuevoJefe = GetWorld()->SpawnActor<ABoss_1>(SpawnLocation, SpawnRotation, SpawnParams); break;
    }
    ...
    if (NuevoJefe)
    {
        ...
        RegistrarEventosBoss(NuevoJefe);
        bJefeDerrotadoNotificado = false;
    }
}
...
{
    if (!bJefeActivo) return;

    TArray<AActor*> BossList;
    UGameplayStatics::GetAllActorsOfClass(GetWorld(), ABoss::StaticClass(), BossList);
